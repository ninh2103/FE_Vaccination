...
const Sidebar: React.FC<SidebarProps> = ({ isCollapsed }) => {
  ...
  const sidebarItems: SidebarItem[] = [
    { name: 'Tổng quan', icon: <CogIcon className='h-5 w-5' />, path: '/admin/dashboard', isDoctor: true },
    { name: 'Loại vắc xin', icon: <ShieldPlus className='h-5 w-5' />, path: '/admin/category', isDoctor: true },
    { name: 'Vắc xin', icon: <IconMedicalCross className='h-5 w-5' />, path: '/admin/vaccines', isDoctor: true },
    ...
    { name: 'Lịch hẹn', icon: <Clock10 className='h-5 w-5' />, path: '/admin/appointments', isDoctor: true },
    ...
    { name: 'Lịch sử', icon: <FolderClock className='h-5 w-5' />, path: '/admin/history', isDoctor: true }
  ]
  ...
  return (
    ...
    >
      <nav className='mt-4 flex-1'>
        {sidebarItems
          .filter((item) => {
            if (isDoctor) return item.isDoctor
            return true
          })
          .map((item, index) => (
          ...
          ${activeItem === item.name || activeSubItem === item.path ? 'bg-gray-100' : 'hover:bg-gray-100'}`}
              >
                <div
