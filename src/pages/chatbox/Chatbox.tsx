...
import { MessageSquare, Send, X, CircleStop, Sun, Moon } from 'lucide-react'
import { GoogleGenerativeAI } from '@google/generative-ai'
import { history } from '@/pages/chatbox/training'

// Note: Move API key to environment variable for security
...
export default function Chatbox() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: Date.now(),
      sender: 'bot',
      text: 'Chào bạn! Tôi là trợ lý ảo của trung tâm tiêm chủng VAXBOT. Nếu có câu hỏi liên quan tới tiêm chủng thì đừng ngại mà đặt câu hỏi cho tôi!',
      data: [],
  ...
  const fetchAIResponse = async (userMessage: string) => {
    try {
      ...
      const isInDepthQuestion = inDepthKeywords.some((keyword) => userMessage.toLowerCase().includes(keyword))
      ...
      if (!isVaccineRelated && userMessage.toLowerCase().includes('xin chào')) {
        botReply = {
          messages: 'Chào bạn! Tôi có thể giúp gì cho bạn về vắc-xin và tiêm chủng không?',
          data: []
        }
      } else {
          ...
          const isValidData =
            ...
            parsed.data.every(
              (item: any) =>
                ...
                'amout' in item &&
                typeof item.amout === 'string' &&
                'redirectlink' in item &&
                typeof item.redirectlink === 'string'
            )
          botReply = {
            messages: parsed.messages || '',
            data: isValidData ? parsed.data : []
          }
        } catch (e) {
          botReply = { messages: responseText, data: [] }
        }

        // Append in-depth notice if the question is vaccine-related and in-depth
        if (isVaccineRelated && isInDepthQuestion) {
          botReply.messages +=
            '\n\nĐể được cung cấp thông tin chính xác nhất, vui lòng liên hệ với nhân viên qua biểu tượng Messenger ở trên.'
        }
      }
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-40 ${userPreferences.theme}`}>
