...
export function SuppliersTable({ onUpdateSuppliers }: SuppliersTableProps) {
  ...
  const handleExport = () => {
    ...
    try {
      ...
      const header = ['STT', 'Mã Nhà cung cấp', 'Tên nhà cung cấp', 'Địa chỉ', 'Số điện thoại']
      ...
      exportData.forEach((row, rowIndex) => {
        ...
        // Mã Nhà cung cấp (căn trái)
        const idCell = XLSX.utils.encode_cell({ r: rowIndex + 7, c: 1 })
        worksheet[idCell].s = {
          font: { sz: 12 },
          alignment: { horizontal: 'left' },
          border: {
        ...
        const nameCell = XLSX.utils.encode_cell({ r: rowIndex + 7, c: 2 })
        ...
        // Địa chỉ (căn trái)
        const addressCell = XLSX.utils.encode_cell({ r: rowIndex + 7, c: 3 })
        ...
        const phoneCell = XLSX.utils.encode_cell({ r: rowIndex + 7, c: 4 })
              ...
              <TableBody>
                {paginatedSuppliers.map((supplier, index) => (
                  <TableRow key={supplier.id} className='cursor-pointer hover:bg-muted/50'>
                    ...
                    <TableCell onClick={() => handleShowDetail(supplier)} className='font-medium'>
                      {supplier.id}
                    </TableCell>
        ...
        <DialogContent className='sm:max-w-[550px]'>
          ...
          <div className='grid gap-4 py-4'>
            <div className='flex flex-col gap-2'>
              <label className='font-medium'>Mã Nhà cung cấp</label>
